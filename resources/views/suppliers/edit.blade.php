                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label for="tin" class="block text-sm font-medium text-gray-700">TIN (BIR
                                Compliance)</label>
                            <input type="text" name="tin" id="tin" value="{{ old('tin', $supplier->tin) }}"
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="address" id="address" rows="3"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('address') @enderror">{{ old('address', $supplier->address) }}</textarea>
                            @error('address')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="payment_terms" id="payment_terms" required
                                ...
                                <option value="COD"
                                    {{ old('payment_terms', $supplier->payment_terms) == 'COD' ? 'selected' : '' }}>Cash on
                                    Delivery (COD)</option>
                                <option value="15 days"
                                    {{ old('payment_terms', $supplier->payment_terms) == '15 days' ? 'selected' : '' }}>15
                                    days</option>
                                <option value="30 days"
                                    {{ old('payment_terms', $supplier->payment_terms) == '30 days' ? 'selected' : '' }}>30
                                    days</option>
                                <option value="45 days"
                                    {{ old('payment_terms', $supplier->payment_terms) == '45 days' ? 'selected' : '' }}>45
                                    days</option>
                                <option value="60 days"
                                    {{ old('payment_terms', $supplier->payment_terms) == '60 days' ? 'selected' : '' }}>60
                                    days</option>
                                <option value="90 days"
                                    {{ old('payment_terms', $supplier->payment_terms) == '90 days' ? 'selected' : '' }}>90
                                    days</option>
                            </select>
                            @error('payment_terms')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <textarea name="bank_details" id="bank_details" rows="3"
                                ...
                                placeholder="Bank Name, Account Number, Account Holder Name">{{ old('bank_details', $supplier->bank_details) }}</textarea>
                            @error('bank_details')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
